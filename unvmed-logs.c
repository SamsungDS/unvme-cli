...
static const char *unvme_log_delete_sq(union nvme_cmd *sqe)
{
...
static const char *unvme_log_create_sq(union nvme_cmd *sqe)
{
	uint16_t sqid = le32_to_cpu(sqe->cdw10) & 0xFFFF;
...
static const char *unvme_log_delete_cq(union nvme_cmd *sqe)
{
...
static const char *unvme_log_create_cq(union nvme_cmd *sqe)
{
	uint16_t cqid = le32_to_cpu(sqe->cdw10) & 0xFFFF;
	uint16_t qsize = (le32_to_cpu(sqe->cdw10) >> 16) & 0xFFFF;
...
static const char *unvme_log_admin_identify(union nvme_cmd *sqe)
{
...
static const char *unvme_log_read_write(union nvme_cmd *sqe)
{
...
static const char *unvme_log_common(union nvme_cmd *sqe)
{
...
static const char *unvme_log_io_cmd(union nvme_cmd *sqe)
{
	switch (sqe->opcode) {
	case nvme_cmd_write:
	case nvme_cmd_read:
		return unvme_log_read_write(sqe);
	}

	return unvme_log_common(sqe);
}

static const char *unvme_log_admin_cmd(union nvme_cmd *sqe)
{
	switch (sqe->opcode) {
	case nvme_admin_delete_sq:
		return unvme_log_delete_sq(sqe);
	case nvme_admin_create_sq:
		return unvme_log_create_sq(sqe);
	case nvme_admin_delete_cq:
		return unvme_log_delete_cq(sqe);
	case nvme_admin_create_cq:
		return unvme_log_create_cq(sqe);
	case nvme_admin_identify:
		return unvme_log_admin_identify(sqe);
	}
