...
static inline void __unvme_cmd_pr(const char *format, struct unvme_cmd *cmd,
			   void (*pr)(const char *format, void *vaddr))
{
	if (streq(format, "binary"))
		unvme_pr_raw(cmd->buf.va, cmd->buf.len);
	else if (streq(format, "normal") || streq(format, "json"))
		pr(format, cmd->buf.va);
	else
		assert(false);
}
...
int unvme_id_ns(int argc, char *argv[], struct unvme_msg *msg)
{
	...
	if (!ret) {
		__unvme_cmd_pr(arg_strv(format), cmd, unvme_pr_id_ns);
...
int unvme_id_ctrl(int argc, char *argv[], struct unvme_msg *msg)
{
	...
	if (!ret) {
		...
		__unvme_cmd_pr(arg_strv(format), cmd, unvme_pr_id_ctrl);
	} else if (ret > 0)
		unvme_pr_cqe_status(ret);
	else
		unvme_pr_err("failed to identify namespace\n");
...
int unvme_id_active_nslist(int argc, char *argv[], struct unvme_msg *msg)
{
	...
	if (!ret)
		__unvme_cmd_pr(arg_strv(format), cmd, unvme_pr_id_active_nslist);
	else if (ret > 0)
		unvme_pr_cqe_status(ret);
	else
...
int unvme_nvm_id_ns(int argc, char *argv[], struct unvme_msg *msg)
{
	...
	if (!ret) {
		...
		__unvme_cmd_pr(arg_strv(format), cmd, unvme_pr_nvm_id_ns);
	}
	else if (ret > 0)
		unvme_pr_cqe_status(ret);
	else
...
int unvme_id_primary_ctrl_caps(int argc, char *argv[], struct unvme_msg *msg)
{
	...
	if (!ret)
		__unvme_cmd_pr(arg_strv(format), cmd, unvme_pr_id_primary_ctrl_caps);
	else if (ret > 0)
		unvme_pr_cqe_status(ret);
	else
...
int unvme_id_secondary_ctrl_list(int argc, char *argv[], struct unvme_msg *msg)
{
	...
	if (!ret)
		__unvme_cmd_pr(arg_strv(format), cmd, unvme_pr_id_secondary_ctrl_list);
	else if (ret > 0)
		unvme_pr_cqe_status(ret);
	else
